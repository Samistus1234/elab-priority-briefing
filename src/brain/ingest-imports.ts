...
export async function runImportIngestion(): Promise<{ processed: number; created: number; reinforced: number; failed: number }> {
  ...
  const { data: imports, error } = await sb.from("brain_imports")
    ...
    .eq("org_id", orgId).eq("status", "pending").order("created_at", { ascending: true });
  ...
  for (const imp of imports ?? []) {
    ...
    try {
      ...
      let cCreated = 0, cReinforced = 0;
      for (const chunk of chunks) {
        try {
          const units = await extractFromTranscript(chunk.transcript);
          ...
          logger.error({ err: (e as Error).message, importId: imp.id }, "brain import: chunk failed");
        }
      }
      ...
      // Delete the raw file — only the scrubbed knowledge persists.
      await sb.storage.from(bucket).remove([imp.storage_path]).then(() => {}, () => {});
      processed++; created += cCreated; reinforced += cReinforced;
    } catch (e) {
      failed++;
      await sb.from("brain_imports").update({
        status: "failed", error: (e as Error).message, processed_at: new Date().toISOString(),
      }).eq("id", imp.id);
      logger.error({ err: (e as Error).message, importId: imp.id }, "brain import: failed");
    }
  }
